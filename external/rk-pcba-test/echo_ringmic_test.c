...
int linear_amp(void *data, int bytes)
{
	int i = 0, j = 0;
	for (i = 0; i < bytes / 2; ) {
		short raw = *((short *)data + i);
		int tmp_data = (int)raw;
#if 1
		tmp_data = tmp_data << 7;
		if(tmp_data > 32767) {
 ...
 */
int ringmic_test_slef(char *result, int flag)
{
	int *record_ret = NULL;
	int fd = 0;
	...
	if (flag) {
		...
		system("busybox killall -9 atinycap");
		system("busybox rm /tmp/ringmic_vibration.pcm");
		system("atinyplay /res/vibration.wav &");
		usleep(200000);
		system("atinycap /tmp/ringmic_vibration.pcm -D 0 -d 1 -r 16000-c 8 -T 15");
		...
		/* Play the specified file, and recording. */
		system("busybox killall -9 atinycap");
		system("busybox rm /tmp/ringmic_record.pcm");
		system("atinymix 0 2");
		system("atinymix 1 1");
		system("atinyplay /res/rectest_400hz.wav &");
		usleep(200000);
		system("atinycap /tmp/ringmic_record.pcm -D 0 -d 1 -r 16000 -c 8 -T 15");
		/* system("arecord -t raw -f S16_LE -c 8 -r 16000 -d 15 /tmp/ringmic_record.pcm");*/
	}
	system("busybox killall -9 atinycap");
	system("busybox killall atinyplay");
	...
	if (!flag) {
		...
		//system("busybox rm -rf /tmp/ringmic_record.pcm");
	} else {
		...
		system("busybox rm -rf /data/ringmic_vibration.pcm");
	}
...
void* ringmic_test(void *argv)
{
	...
	printf ("ringmic_test entering\n");
	if(tc_info->y <= 0)
		tc_info->y  = get_cur_print_y();
	ui_print_xy_rgba(0,tc_info->y,255,255,0,255,"%s Testing\n",PCBA_RINGMIC);
	sleep(3);

	printf ("BEGIN %s\n", PCBA_RINGMIC);
	if(!ringmic_test_slef(record_ret, 0)) {
		i = sprintf(msg, "bad:[");
		for(int idx = 0; idx < AUDIO_CHANNAL_CNT; idx++) {
			if(record_ret[idx]) {
				i  += sprintf(&msg[i], "#%d ", idx);
				ret = -1;
			}
		}
		i  += sprintf(&msg[i ], "]");

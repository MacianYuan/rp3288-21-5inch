...
int rpdzkj_config_write(struct file * file,const char __user* buffer,size_t count,loff_t *data)
{
        return count;
}
...
static int STRING_READ(x)(struct file *file, char __user *buf,size_t count, loff_t *ppos)\
{\
...
static int rpdzkj_cfg_probe(struct platform_device *pdev)
{
	...
	STRING_READ_DTS(module_4G,"EC20",temp)

...
//add by rpdzkj yang for long push volumeup toggle usb mode 20190320
long int touch_wakeup_time(void)
{
        struct timespec tm;
...
long int time_difference_absolute(long int time_val)
{
        long time_temp = 0;

        time_temp = touch_wakeup_time() - time_val;
...
static int usb_mode_changingOver(struct rk_keys_button *button)
{
	static long int touch_wakeup_timeVal = 0;

	struct file *ffp = NULL;
	char usb_mode_patch[] = "/sys/bus/platform/drivers/usb20_otg/force_usb_mode";
	int fileRet = -1;

	char usb_host_mode_state[] = "1";
	char usb_device_mode_state[] = "2";

	char curr_usb_mode_state[2] = {-1, -1};


//	if(button->code == KEY_VOLUMEUP)
	if(button->state == 0){ //vol+ up
		if(time_difference_absolute(touch_wakeup_timeVal) > 2){ //4s or exceed 4s
			fileRet = openFile(&ffp, usb_mode_patch, O_CREAT|O_RDWR, 0666);
			if(fileRet == 0 && ffp != NULL){
				fileRet = readFile(ffp, curr_usb_mode_state, 2);
				if(fileRet == 0 || fileRet > 0){
					if ('1' == curr_usb_mode_state[0]){
						writeFile(ffp, usb_device_mode_state, 1); //usb device
					} else if ('2' == curr_usb_mode_state[0]){
						writeFile(ffp, usb_host_mode_state, 1); //usb host
					} else { 			//'0' == curr_usb_mode_state[0]
						writeFile(ffp, usb_host_mode_state, 1);
					}
				}
			closeFile(ffp);
			}
		}
	}else{ //button->state == 0  vol+ down
		touch_wakeup_timeVal = touch_wakeup_time();
	}
//	}

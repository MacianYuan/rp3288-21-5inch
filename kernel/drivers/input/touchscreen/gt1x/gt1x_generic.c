...
s32 gt1x_init_panel(void)
{
	...
	u8 cfg_len = 0;
//rpdzkj tp property for dts
if(gtp_config->gtp_send_cfgs == 1){//#if GTP_DRIVER_SEND_CFG
	u8 sensor_id = 0;
	...
	gt1x_config[RESOLUTION_LOC] = (u8) gtp_config->gtp_resolution_x;
	gt1x_config[RESOLUTION_LOC + 1] = (u8) (gtp_config->gtp_resolution_x >> 8);
	gt1x_config[RESOLUTION_LOC + 2] = (u8) gtp_config->gtp_resolution_y;
	gt1x_config[RESOLUTION_LOC + 3] = (u8) (gtp_config->gtp_resolution_y >> 8);
...
//rpdzkj tp property for dts
				...
				input_x = gt1x_abs_x_max - 1 - input_x;
				input_y = input_y;
				}
				if (gtp_config->gtp_overturn_y == 1){
				input_x = input_x;
				input_y = gt1x_abs_y_max - 1 - input_y;
				}
//rpdzkj tp property for dts

...
static int rt5640_parse_dt(struct rt5640_priv *rt5640, struct device_node *np)
{
	...
	of_property_read_u32(np,"play-volume",(u32 *)&(rt5640->pdata.play_volume));
	of_property_read_u32(np,"capture-volume",(u32 *)&(rt5640->pdata.capture_volume));
		    ...
		    const struct i2c_device_id *id)
{
	...
	if (rt5640->pdata.in3_diff)
        ...
        if (rt5640->pdata.play_volume >= 0) {
		temp = (rt5640->pdata.play_volume << 8) | rt5640->pdata.play_volume;
                regmap_update_bits(rt5640->regmap, RT5640_DAC2_DIG_VOL,
                                        0xffff, temp);
	}

        if (rt5640->pdata.capture_volume >= 0) {
		temp = (rt5640->pdata.capture_volume << 8) | rt5640->pdata.capture_volume;

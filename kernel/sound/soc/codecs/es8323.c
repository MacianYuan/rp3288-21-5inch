...
#define es8323_IN_DEF_VOL	0xc0 //0 max 0xc0 min
#define SPK_OPEN_NUM 2
...
static int es8323_set_gpio_start(int gpio, bool level)
{
...
static int es8323_set_gpio(int gpio, bool level)
{
	...
	if ((gpio & ES8323_CODEC_SET_SPK) && es8323
		...
		es8323_set_gpio_start(es8323->spk_ctl_gpio, level);
	}
...
static const struct snd_kcontrol_new es8323_snd_controls[] = {
	...
	SOC_SINGLE_TLV("Left Channel Capture Volume", ES8323_ADCCONTROL1, 4, 24,
		       0, bypass_tlv),
...
static int es8323_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		...
		snd_soc_write(codec, 0x10, es8323_IN_DEF_VOL);
		snd_soc_write(codec, 0x11, es8323_IN_DEF_VOL);

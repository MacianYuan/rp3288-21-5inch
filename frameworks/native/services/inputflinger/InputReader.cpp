...
//add by rpdzkj yang for tp rotate Orientation 20181218
...
//add by rpdzkj yang for tp rotate Orientation 20181218
int TpOrientation = 0;
char SystemRotatepropValue[PROPERTY_VALUE_MAX];
char BootAnimationpropValue[PROPERTY_VALUE_MAX];
bool LogIntoSystemRotate = false;
bool FirstInRotate = false;
//end
...
//add by rpdzkj yang for tp rotate Orientation 20181218
bool getBootAnimationPorpState()
{
	
    property_get("init.svc.bootanim", BootAnimationpropValue, "bootanim");
	if (!strcmp(BootAnimationpropValue, "stopped"))  
	{
		ALOGD("yang, getBootAnimationPorpState() stopped");
		LogIntoSystemRotate = true;
		return LogIntoSystemRotate;
	}else{
		ALOGD("yang, getBootAnimationPorpState() running");
		LogIntoSystemRotate = false;
		return LogIntoSystemRotate;
	}	
}

int getporpSystemRotate()
{
    property_get("ro.sf.hwrotation", SystemRotatepropValue, "hwrotation");
    switch(atoi(SystemRotatepropValue)) {
	case 0:
	    TpOrientation = DISPLAY_ORIENTATION_0;
	    break;
	case 90:
	    TpOrientation = DISPLAY_ORIENTATION_90;
	    break;
	case 180:
	    TpOrientation = DISPLAY_ORIENTATION_180;
	    break;
	case 270:
	    TpOrientation = DISPLAY_ORIENTATION_270;
	    break;
    }
//	mViewport.orientation = TpOrientation;

    return TpOrientation;
}

void tpOrientationConfig(int *mViewportOrientation )
{
	if(TpOrientation != DISPLAY_ORIENTATION_0)
	{
		*mViewportOrientation = *mViewportOrientation + TpOrientation;
		       if(*mViewportOrientation > DISPLAY_ORIENTATION_270)
				*mViewportOrientation = *mViewportOrientation - (DISPLAY_ORIENTATION_270 + 1);
	}
}
//end
...
void TouchInputMapper::configureSurface(nsecs_t when, bool* outResetNeeded) {
    ...
    ALOGD("rpdzkj jeff yang, InputReader.cpp TouchInputMapper::configureSurface() rawWidth:%d  rawHeight %d",rawWidth, rawHeight);
...
//add by rpdzkj yang for tp rotate Orientation 20181218
#if 1
	if(/*false == LogIntoSystemRotate*/false){		
		mViewport.orientation = getporpSystemRotate();
		
		if(true == getBootAnimationPorpState()){
			tpOrientationConfig(&(mViewport.orientation));
			ALOGD("3333rpdzkj yang, InputReader.cpp TouchInputMapper::configureSurface() Line:3925 true == TimeDiffCalculate(), mViewport.orientation:%d", mViewport.orientation);//add by yang
		}
	} else {
	    ALOGD("yang, InputReader.cpp TouchInputMapper::configureSurface() Line:3925 (LogIntoSystemRotate == true) 000");
	    if (false == FirstInRotate)
	    	{
	    	mViewport.orientation = getporpSystemRotate();
	    	FirstInRotate = true;
	    }
...
//end
	...
	mViewport.orientation = -1; //add by rpdzkj yang for tp rotate Orientation 20181218
        } else {
    ...
    if (viewportChanged || deviceModeChanged) {
        ALOGI("rpdzkj jeff Device reconfigured: id=%d, name='%s', size %dx%d, orientation %d, mode %d, "
                "display id %d",

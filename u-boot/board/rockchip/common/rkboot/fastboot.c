...
void board_fbt_preboot(void)
{
	...
	struct fdt_gpio_state sd_gpio_pwr_hold;
	struct fdt_gpio_state v33_gpio_pwr_hold;
	struct fdt_gpio_state v5_gpio_pwr_hold;
	int sd_node = fdt_path_offset(gd->fdt_blob, "/33v_power_en");
	sd_gpio_pwr_hold.gpio = -1;
	v33_gpio_pwr_hold.gpio = -1;
	v5_gpio_pwr_hold.gpio = -1;
	
	fdtdec_decode_gpios(gd->fdt_blob, sd_node, "5ven_pin", &v5_gpio_pwr_hold, 1);
	res = gpio_direction_output(v5_gpio_pwr_hold.gpio, 1);
	if(res < 0){
		printf("eerrrorrrooo 1111111111111111\n");
	}

	fdtdec_decode_gpios(gd->fdt_blob, sd_node, "33ven_pin", &v33_gpio_pwr_hold, 1);
	res = gpio_direction_output(v33_gpio_pwr_hold.gpio, 1);
	if(res < 0){
		printf("eerrrorrrooo 2222222222222222\n");
	}
	gpio_set_value(v33_gpio_pwr_hold.gpio, 1);

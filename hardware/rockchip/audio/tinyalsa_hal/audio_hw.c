...
static int read_snd_card_info(void)
{
    ...
    if (strstr (buf1, "SPDIF") || strstr (buf1, "rockchipspdif")) {
		...
		PCM_CARD = 0;
		PCM_CARD_SPDIF = 1;
	}else if(strstr(buf0,"es8323codec") || strstr(buf1, "es8323-codec")){
		if(strstr(buf1, "hdmispdif")){
        	PCM_CARD =0;
			PCM_CARD_SPDIF = 1;
        	PCM_CARD_HDMI = 1;
		}
	}else if(strstr(buf0,"rt5631codec")){
		PCM_CARD_SPDIF = 0;
        	PCM_CARD =1;
        	PCM_CARD_HDMI = 1;
	}else if(strstr(buf0,"hdmispdif")){
			PCM_CARD_SPDIF = 0;
        	PCM_CARD =1;
        	PCM_CARD_HDMI = 0;
	}
...
#ifdef CORE_AND_HDMI
    if ((out->device & AUDIO_DEVICE_OUT_AUX_DIGITAL) && !(out->device & (AUDIO_DEVICE_OUT_SPEAKER | AUDIO_DEVICE_OUT_WIRED_HEADSET |  AUDIO_DEVICE_OUT_WIRED_HEADPHONE))){
#else
    if ((out->device & AUDIO_DEVICE_OUT_AUX_DIGITAL) && !(out->device & (AUDIO_DEVICE_OUT_WIRED_HEADSET |  AUDIO_DEVICE_OUT_WIRED_HEADPHONE))){
#endif
        if (connect_hdmi) {
#ifdef BOX_HAL
#ifdef USE_DRM
...
#ifdef CORE_AND_HDMI
		if (connect_hdmi) {
			if((out->device & AUDIO_DEVICE_OUT_SPEAKER) && !(out->device & (AUDIO_DEVICE_OUT_WIRED_HEADSET |  AUDIO_DEVICE_OUT_WIRED_HEADPHONE))){
				out->pcm[PCM_CARD_HDMI] = pcm_open(PCM_CARD_HDMI, out->pcm_device,
		                                                PCM_OUT | PCM_MONOTONIC, &out->config);
		            if (out->pcm[PCM_CARD_HDMI] &&
		                    !pcm_is_ready(out->pcm[PCM_CARD_HDMI])) {

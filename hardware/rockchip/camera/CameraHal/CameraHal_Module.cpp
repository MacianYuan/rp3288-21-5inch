...
int camera_get_number_of_cameras(void)
{
    ...
    int hwrotation = 0;
    int back_cam_rotation = 0;
    int front_cam_rotation = 0;
   	...
   	if(cam_cnt<CAMERAS_SUPPORT_MAX){
        ...
        for (i=0; i<10; i++) {
			...
			camInfoTmp[cam_cnt].facing_info.facing = CAMERA_FACING_BACK;
                	...
                	if (camInfoTmp[cam_cnt-1].facing_info.facing == CAMERA_FACING_FRONT){
                	        camInfoTmp[cam_cnt].facing_info.facing = CAMERA_FACING_BACK;
                	}else{  
		...
		property_get("ro.sf.only.front", property, "false");
...
//rpdzkj jeff,for rotation
#if 0
	property_get("ro.sf.back.camera.rotation", property, "0");
	back_cam_rotation = strtol(property,0,0);
        gCamInfos[0].facing_info.orientation = back_cam_rotation;
	
	property_get("ro.sf.front.camera.rotation", property, "0");
	front_cam_rotation = strtol(property,0,0);
        gCamInfos[1].facing_info.orientation = front_cam_rotation;
#else
	if( camInfoTmp[0].facing_info.facing == CAMERA_FACING_BACK){
		property_get("ro.sf.back.camera.rotation", property, "0");
		back_cam_rotation = strtol(property,0,0);
		gCamInfos[0].facing_info.orientation = back_cam_rotation;
	}       
	if( camInfoTmp[1].facing_info.facing == CAMERA_FACING_BACK){
		property_get("ro.sf.back.camera.rotation", property, "0");
		back_cam_rotation = strtol(property,0,0);
		gCamInfos[1].facing_info.orientation = back_cam_rotation;
	}

	if( camInfoTmp[0].facing_info.facing == CAMERA_FACING_FRONT){
		property_get("ro.sf.front.camera.rotation", property, "0");
		front_cam_rotation = strtol(property,0,0);
		gCamInfos[0].facing_info.orientation = front_cam_rotation;
	}
	if( camInfoTmp[1].facing_info.facing == CAMERA_FACING_FRONT){
		property_get("ro.sf.front.camera.rotation", property, "0");
		front_cam_rotation = strtol(property,0,0);
		gCamInfos[1].facing_info.orientation = front_cam_rotation;
	}

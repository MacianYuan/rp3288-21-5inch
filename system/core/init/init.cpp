...
void rp_change_properties()
{
	...
	//system_rotate
	char buf[3] ={'\0'};
	FILE *fp = fopen("/proc/rp_prop/system_rotate", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/system_rotate error";
		property_set("ro.sf.hwrotation", "0");
	}else{
		fgets(s, sizeof(s), fp);
		fclose(fp); 
		property_set("ro.sf.hwrotation", s);
		memset(s,'\0',sizeof(s));
	}

	//back_camera_rotate
	fp = fopen("/proc/rp_prop/back_camera_rotate", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/back_camera_rotate error";
		property_set("ro.sf.back.camera.rotation", "0");
	}else{
		fgets(s, sizeof(s), fp);
		fclose(fp); 
		property_set("ro.sf.back.camera.rotation", s);
		memset(s,'\0',sizeof(s)); 
	}
	
	//front_camera_rotate
	fp = fopen("/proc/rp_prop/front_camera_rotate", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/front_camera_rotate error";
		property_set("ro.sf.front.camera.rotation", "0");
	}else{
		fgets(s, sizeof(s), fp);
		fclose(fp); 
		property_set("ro.sf.front.camera.rotation", s);
		memset(s,'\0',sizeof(s));
	}

	//lcd_density
	fp = fopen("/proc/rp_prop/lcd_density", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/lcd_density error";
		property_set("ro.sf.lcd_density", "240");
	}else{
		fgets(s, sizeof(s), fp);
		fclose(fp); 
		property_set("ro.sf.lcd_density", s);
		memset(s,'\0',sizeof(s)); 
	}
	
	//language
	fp = fopen("/proc/rp_prop/language", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/language error";
		...
		fclose(fp);
		buf[0] = s[0];   buf[1] = s[1];
		property_set("ro.product.locale.language", buf);
		buf[0] = s[3];   buf[1] = s[4];
		property_set("ro.product.locale.region", buf);
		property_set("ro.product.locale", s);
		memset(s,'\0',sizeof(s)); 
	}
	
	//time_zone
	fp = fopen("/proc/rp_prop/time_zone", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/timezone error";
		property_set("persist.sys.timezone", "en-US");
	}else{
		fgets(s, sizeof(s), fp);
		fclose(fp);
		property_set("persist.sys.timezone", s);
		memset(s,'\0',sizeof(s)); 
	}

	//not_navigation_bar
	fp = fopen("/proc/rp_prop/not_navigation_bar", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/not_navigation_bar error";
	...
	//status_bar
	fp = fopen("/proc/rp_prop/not_status_bar", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/not_status_bar error";
		property_set("ro.rpdzkj.no.statusbar", "false");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("ro.rpdzkj.no.statusbar", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}
	
	//default_launcher
	fp = fopen("/proc/rp_prop/default_launcher", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/default_launcher error";
		property_set("rpdzkj.default.launcher", "true");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("rpdzkj.default.launcher", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}

	//has_root
	fp = fopen("/proc/rp_prop/has_root", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/has_root error";
		property_set("rpdzkj.has.root", "false");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("rpdzkj.has.root", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}

	//usb_not_permission
	fp = fopen("/proc/rp_prop/usb_not_permission", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/usb_not_permission error";
		property_set("ro.rpdzkj.no.usb.permission", "true");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("ro.rpdzkj.no.usb.permission", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}

	//usb_camera_only_front
	fp = fopen("/proc/rp_prop/usb_camera_only_front", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/usb_camera_only_front error";
		property_set("ro.sf.only.front", "false");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("ro.sf.only.front", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}
	
	//gps_use
	fp = fopen("/proc/rp_prop/gps_use", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/gps_use error";
		property_set("ro.factory.hasGPS", "false");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("ro.factory.hasGPS", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}
	
	//gps_serial_port
	fp = fopen("/proc/rp_prop/gps_serial_port", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/gps_serial_port error";
		property_set("ro.sf.gps.serial", "/dev/ttyS4");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("ro.sf.gps.serial", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}
	
	//user_version
	fp = fopen("/proc/rp_prop/user_version", "r" );
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/user_version error";
		property_set("ro.sf.user_version", "rpdzkj");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("ro.sf.user_version", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}
	
	//primary_device
	fp = fopen("/proc/rp_prop/primary_device", "r");
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/primary_device error";
	}else{
		fgets(s, sizeof(s), fp);
		if(!strcmp(s,"null")){
		fclose(fp);
		...
		property_set("sys.hwc.device.primary", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
		}
	}
	//extend_device
	fp = fopen("/proc/rp_prop/extend_device", "r");
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/extend_device error";
	}else{
		fgets(s, sizeof(s), fp);
		if(!strcmp(s,"null")){
		fclose(fp);
		...
		property_set("sys.hwc.device.extend", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s));
		} 
	}
	//extend_rotate
	fp = fopen("/proc/rp_prop/extend_rotate", "r");
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/extend_rotate error";
		property_set("persist.sys.rotation.einit", "0");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("persist.sys.rotation.einit", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}

	//rotation_efull
	fp = fopen("/proc/rp_prop/rotation_efull", "r");
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/rotation_efull error";
		property_set("persist.sys.rotation.efull", "false");
	}else{
		fgets(s, sizeof(s), fp);
		property_set("persist.sys.rotation.efull", s);
		fclose(fp); 
		memset(s,'\0',sizeof(s)); 
	}
	
	//home_apk
	fp = fopen("/proc/rp_prop/home_apk", "r");
	if(fp==NULL){
		PLOG(ERROR) << "open /proc/rp_prop/home_apk error";
	}else{
		fgets(s, sizeof(s), fp);
		if(!strcmp(s,"null")){
		fclose(fp);
...
int main(int argc, char** argv) {
    ...
    PLOG(ERROR) << "reading config file";
